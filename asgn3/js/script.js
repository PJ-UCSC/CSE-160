...
function startNextWave() {
    ...
    spawnZombies(currentWave == 2 ? 2 : 5);
}
...
function tick() {
    ...
    for (let z of zombies) {
        ...
        if (dist > 0.5) { z.pos[0] += (dx/dist)*z.speed; z.pos[2] += (dz/dist)*z.speed; }
        else { camera.hp -= 0.4; document.getElementById('hp-bar').style.width = Math.max(0, camera.hp) + "%"; }
    }
    if (camera.hp <= 0) { camera.isDead = true; document.getElementById('game-over').style.display = 'flex'; }
